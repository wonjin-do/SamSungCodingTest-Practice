...
struct priorityQueue {
	...
	void push(int x) {
		data[len++] = x;//마지막 노드에 newData 추가
		int idx = len - 1;//옮겨질 새로운 데이터의 위치정보를 수시로 이 변수에 업뎃할것임
		while (idx > 1 && data[idx] < data[idx / 2]) { //루트노드가 아닐때만
			int temp = data[idx];
			data[idx] = data[idx / 2];
			data[idx / 2] = temp;
			idx = idx / 2; // 자리바꾸면 업뎃!
	...
	void pop() {
		...
		int idx = 1;
		while (1) {//현재 idx의 위치에서 자식들과 교체할 수 있는지 비교
			...
			//pIdx를 찾는 과정이 길기 때문에 while문의 조건절을 true로 해줘야 함.
